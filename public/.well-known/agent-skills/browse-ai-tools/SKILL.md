...
```
GET https://enclavetools.com/llms.txt
Accept: text/plain
```
...
```
GET https://enclavetools.com/{category}
```

Example: https://enclavetools.com/code
...
```
GET https://enclavetools.com/.well-known/api-catalog
...
- Respect the `Content-Signal: ai-train=no, search=yes, ai-input=yes` policy
  declared in `https://enclavetools.com/robots.txt`.
