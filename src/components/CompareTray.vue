  ...
  <Teleport to="body">
    <div v-if="selected.length >= 2 || limitHit" class="fixed bottom-0 left-0 right-0 bg-brand-forest text-white p-4 flex flex-col md:flex-row md:justify-between md:items-center gap-3 z-[100]" role="status">
      ...
      <div class="flex gap-4 items-center shrink-0">
        ...
        <a v-if="selected.length >= 2 && !isComparePage" :href="compareUrl" class="h-10 inline-flex items-center bg-brand-forest text-white px-4 font-bold text-xs uppercase tracking-wider no-underline hover:bg-black transition-colors border-2 border-white rounded-full shadow-sm">View comparison</a>
      </div>
    </div>
  </Teleport>
</template>
