...
<template>
  <div v-if="selected.length >= 2 || limitHit" class="fixed bottom-0 left-0 right-0 bg-gray-900 text-white p-4 flex justify-between items-center z-[100]" role="status">
    ...
    <div class="flex gap-4 items-center">
      ...
      <a v-if="selected.length >= 2" :href="compareUrl" class="bg-primary-500 text-white px-4 py-2 font-bold no-underline hover:bg-yellow-400 hover:text-gray-900 transition-colors rounded-lg">View comparison</a>
    </div>
  </div>
</template>
