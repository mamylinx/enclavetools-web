<template>
  <div>
    <div v-if="success" class="flex flex-col items-center text-center gap-6 py-8">
      <div
        class="w-20 h-20 bg-green-600 text-white flex items-center justify-center text-4xl font-extrabold border-4 border-green-800 shadow-sm">
        ✓</div>
      <div>
      ...
      <div class="bg-brand-bg border border-brand-forest/10 p-6 text-left w-full max-w-md mt-4">
        ...
        <div class="flex flex-col gap-4">
          <div v-for="(step, i) in steps" :key="i" class="flex items-start gap-3">
            <span
              class="w-8 h-8 bg-brand-forest text-white flex items-center justify-center font-extrabold text-xs shrink-0 rounded-full">{{
              i + 1 }}</span>
            <div>
      ...
      <a href="/"
        class="inline-flex items-center justify-center px-8 h-12 font-extrabold uppercase tracking-wider text-sm transition-colors cursor-pointer bg-white border border-brand-forest/10 text-brand-forest hover:bg-brand-teal hover:text-white hover:border-brand-teal no-underline rounded-full">←
        Back to Browse</a>
    </div>
    ...
    <div v-else-if="info" class="flex flex-col gap-6">
      ...
      <button @click="resetForm"
        class="self-start inline-flex items-center justify-center px-6 h-12 font-extrabold uppercase tracking-wider text-sm transition-colors cursor-pointer bg-white border border-brand-forest/10 text-brand-forest hover:bg-brand-teal hover:text-white hover:border-brand-teal rounded-full">Submit
        Another URL</button>
    </div>
    ...
    <form v-else @submit.prevent="submitUrl" class="flex flex-col gap-6">
      <div class="flex flex-col gap-2">
        ...
        <div
          class="flex items-center border h-14 focus-within:ring-2 focus-within:ring-brand-teal transition-all duration-150"
          :class="!url ? 'border-brand-forest/10' : isValid ? 'border-green-500' : 'border-red-500'">
      ...
      <div class="flex flex-wrap gap-3">
        <span v-for="d in domains" :key="d.host"
          ...
          :class="detected === d.host ? 'border-brand-teal text-brand-teal' : 'bg-white border-brand-forest/20 text-brand-muted'">{{
            d.label }}</span>
      </div>
      ...
      <button type="submit" :disabled="!isValid || isSubmitting"
        class="w-full h-14 inline-flex items-center justify-center gap-3 font-extrabold text-xs uppercase tracking-wider text-base transition-all duration-150 cursor-pointer border disabled:opacity-50 disabled:cursor-not-allowed bg-brand-forest text-white border-brand-forest hover:bg-brand-teal hover:border-brand-teal rounded-full">
