<template>
  <div>
    <div v-if="success" class="flex flex-col items-center text-center gap-6 py-8">
      <div class="w-20 h-20 bg-green-600 text-white flex items-center justify-center text-4xl font-extrabold border-4 border-green-800 shadow-sm">✓</div>
      <div>
        <h2 class="text-2xl font-extrabold text-brand-forest mb-3 uppercase tracking-wide">Submitted!</h2>
        <p class="text-brand-muted font-bold max-w-md mx-auto">Your tool is now in our review queue. We'll check it and publish it soon.</p>
      </div>
      <div class="bg-brand-bg border border-brand-forest/10 p-6 text-left w-full max-w-md mt-4">
        ...
        <div class="flex flex-col gap-4">
          <div v-for="(step, i) in steps" :key="i" class="flex items-start gap-3">
            <span class="w-8 h-8 bg-brand-forest text-white flex items-center justify-center font-extrabold text-xs shrink-0 rounded-full">{{ i + 1 }}</span>
            <div>
      ...
      <a href="/" class="inline-flex items-center justify-center px-8 h-12 font-extrabold uppercase tracking-wider text-sm transition-colors cursor-pointer bg-white border border-brand-forest/10 text-brand-forest hover:bg-brand-teal hover:text-white hover:border-brand-teal no-underline rounded-full">← Back to Browse</a>
    </div>
    ...
    <div v-else-if="info" class="flex flex-col gap-6">
      <div class="flex flex-col items-start gap-4 p-6" :class="info.bg">
        <div class="flex items-start gap-4 w-full">
          ...
          <div class="flex-1">
            <h3 class="text-lg font-extrabold mb-1 uppercase tracking-wide" :class="info.titleColor">{{ info.title }}</h3>
            <p class="font-bold text-sm m-0" :class="info.textColor">{{ info.message }}</p>
            <a v-if="info.link" :href="info.link" class="inline-flex items-center gap-1 mt-3 font-extrabold text-sm uppercase tracking-wider underline hover:no-underline">View Tool<svg class="w-4 h-4 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><path d="M5 12h14M12 5l7 7-7 7" /></svg></a>
          </div>
        </div>
      </div>
      <button @click="resetForm" class="self-start inline-flex items-center justify-center px-6 h-12 font-extrabold uppercase tracking-wider text-sm transition-colors cursor-pointer bg-white border border-brand-forest/10 text-brand-forest hover:bg-brand-teal hover:text-white hover:border-brand-teal rounded-full">Submit Another URL</button>
    </div>

    <form v-else @submit.prevent="submitUrl" class="flex flex-col gap-6">
      <div class="flex flex-col gap-2">
        <label for="repoUrl" class="font-extrabold text-brand-forest uppercase tracking-wider text-sm">Tool Repository URL *</label>
        <div class="flex items-center border h-14 px-4 focus-within:ring-2 focus-within:ring-brand-teal focus-within:border-brand-teal transition-all duration-150" :class="!url ? 'border-brand-forest/10' : isValid ? 'border-green-500' : 'border-red-500'">
          <span class="w-6 h-6 mr-3 flex items-center justify-center shrink-0" :class="!url || isSubmitting ? 'text-brand-muted' : isValid ? 'text-green-600' : 'text-red-500'">{{ !url ? '🔗' : isValid ? '✓' : '✗' }}</span>
          <input type="url" id="repoUrl" v-model="url" @input="validateUrl" placeholder="https://github.com/owner/repo" class="flex-1 h-full bg-transparent border-none outline-none font-bold text-brand-forest placeholder:text-brand-muted text-base p-0 m-0" />
        </div>
      ...
      <div class="flex flex-wrap gap-3">
        <span v-for="d in domains" :key="d.host" class="inline-flex items-center px-3 h-8 text-xs font-extrabold uppercase tracking-wider border transition-all duration-150 rounded-full" :class="detected === d.host ? 'bg-brand-tealLight border-brand-teal text-brand-teal' : 'bg-white border-brand-forest/20 text-brand-muted'">{{ d.label }}</span>
      </div>
      ...
      <button type="submit" :disabled="!isValid || isSubmitting" class="w-full h-14 inline-flex items-center justify-center gap-3 font-extrabold uppercase tracking-wider text-base transition-all duration-150 cursor-pointer border disabled:opacity-50 disabled:cursor-not-allowed bg-brand-forest text-white border-brand-forest hover:bg-brand-teal hover:border-brand-teal rounded-full">
