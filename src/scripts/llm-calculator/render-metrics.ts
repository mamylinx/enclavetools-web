...
/** Build the "How much memory you need" card — formula box is now collapsible. */
export function renderMetrics(s: CalcState, mMem: number, kvMem: number, total: number, tSeq: number): string {
  return `
    <div class="card">
      ...
      <div class="info-box text-xs mt-4 mb-1">
        ...
        ${kvMem > mMem ? '<strong class="text-amber-600">can be larger than the model itself</strong>' : 'is within model memory bounds'} — that's normal at scale. This memory can outweigh the model.
      </div>
    </div>`;
}
