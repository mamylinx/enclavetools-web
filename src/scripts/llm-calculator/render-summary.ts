...
/** Build the hero answer card — total VRAM + first GPU recommendation. */
export function renderHeroAnswer(total: number, gpus: GpuRow[]): string {
  let gpuLine = '';
  let gpuCount = '';
  if (gpus.length > 0) {
    const needed = Math.ceil(total / gpus[0].vram);
    gpuCount = `${needed}×`;
    gpuLine = `<div class="hm-sub">${needed}× ${gpus[0].name} (${gpus[0].vram} GB each)</div>`;
    if (gpus.length > 1) {
      gpuLine += `<div class="hm-sub">+ ${gpus.length - 1} more GPU configs in breakdown below</div>`;
    }
  }

  return `
    <div class="card mb-3">
      <p class="sec-hdr">Your hardware needs</p>
      <div class="grid grid-cols-2 gap-4">
        <div class="hero-metric">
          <div class="hm-val">${fmt(total)}</div>
          <div class="hm-lbl">Total memory needed</div>
        </div>
        <div class="hero-metric">
          <div class="hm-val">${gpuCount || '—'}</div>
          <div class="hm-lbl">GPUs required</div>
          ${gpuLine || '<div class="hm-sub">Add GPUs below to see count</div>'}
        </div>
      </div>
    </div>`;
}
    ...
    return `
      <div class="card-warn">
        ...
        <div class="warn-box text-xs">
           From the fine-tuning memory table — exact value for the ${s.params}B model using ${modeLabel}.<br/>
          Training usually runs in separate sessions; the same GPUs can run the AI afterward.
        </div>
      </div>`;
  }
  return `<div class="card-warn"><p class="text-amber-600 text-sm font-semibold">Fine-tuning memory estimate not available</p><p class="text-amber-700 text-xs mt-1">The fine-tuning memory table only covers 7B, 13B, 30B, 70B, and 110B models. This model size is not in the table.</p></div>`;
}
...
export function renderTco(gpuList: GpuRow[], rps: number): string {
  ...
  if (gpuList.length === 0) {
    ...
    if (steps.length > 0) {
      body = `<div class="warn-box text-xs">Cost per 1M prompts will show inside each GPU card once you provide:<br/>${steps.join('<br/>')}</div>`;
    } else {
      body = `<div class="ok-box text-xs">✓ All inputs provided — cost per 1M prompts is shown inside each GPU below.</div>`;
    }
  }
  ...
  return `
    <div class="card">
      ...
      <table>
        ...
        ${rps > 0 ? `<tr><td class="text-brand-muted">Requests per second</td><td>Average: ${rps} req/s · Peak: ${peak95 ? peak95.toFixed(2) : '—'} req/s</td></tr>` : ''}
